...
bool publishMetaData()
{
  if (wifiClient.connected() && mqttClient.connected())
  {
    ...
    success &= mqttClient.publish("ir-powermeter/" METER_NAME "/temperature_c", buffer, false);
    sprintf(buffer, "%lu", millis());
    success &= mqttClient.publish("ir-powermeter/" METER_NAME "/uptime_ms", buffer, false);
    return success;
  }
  return false;
}
